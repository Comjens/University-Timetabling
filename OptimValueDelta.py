...
def Set_obj_delta_slow(data,swap1,swap2):
    temp_set_ctr=[[[1 if ((i, t, r) != swap1[0] and (i, t, r) != swap2[0] and data.timetable[(i, t, r)]) or (i, t, r) ==
                         swap1[1] or (i, t, r) == swap2[1] else 0 for r in range(data.rooms_max)] for t in range(data.total_timeslots)] for i in range(data.Courses_max)]
...
def Set_obj_delta(data, swap1, swap2):
    ...
    for c in range(data.Courses_max):
        for r in range(data.rooms_max):
            RO[r] = sum(temp_set_ctr[c][t][r] for t in range(data.total_timeslots)) - 1
            # for c in range(data.Courses_max):
        P_c[c] = max(0, sum(1 if RO[r] >= 1 else 0 for r in range(data.rooms_max)))
    # data.set_P_c(P_c)
...
import numpy as np
def Set_obj_delta_np(data, swap1, swap2):
    ...
    for c in range(data.Courses_max):
        a = 0;
        for d in range(data.days_max):
            if (np.sum(temp_set_ctr[c][t][r] for r in range(data.rooms_max) for t in
                    data.T_d[d]) >= 1):
                a = a + 1
        Workingdays_c[c] = max(0, (data.M_c[c] - a))
    ...
    obj = np.sum(5 * Workingdays_c[c] + 10 * Unplanned_c[c] + P_c[c] for c in range(data.Courses_max))\
    + np.sum(V_tr[t][r] for t in range(data.total_timeslots) for r in range(data.rooms_max))\
    + 2 * np.sum(A_qt[q][t] for q in range(data.Curricula_max) for t in range(data.total_timeslots))
    return obj
