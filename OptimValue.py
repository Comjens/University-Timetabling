...
def Set_obj(data):
    ...
    for i in range(data.Courses_max):
        Unplanned_c[i] = max(0, data.L_c[i] - sum(data.timetable[(i,t,r)] for t in range(data.total_timeslots) for r in range(data.rooms_max)) )
    ...
    for t in range(data.total_timeslots): 
        for r in range(data.rooms_max):
            V_tr[t][r] = max (0, sum ([data.timetable[(c,t,r)]*data.S_c[c] for c in range(data.Courses_max)]) - data.C_r[r])
    data.set_V_tr(V_tr)
    #print(V_tr)
