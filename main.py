...
def Optimize(que,initList):
    while True:
        try:
            params = {i:j for i,j in zip(['Set', 'Alpha', 'Beta', 'Gamma', 'Delta', 'Sigma', 'Epsilon', 'Initiate'],initList[random.randint(1,len(initList)-1)])}
            ...
            TotLength = min(data.rooms_max * data.total_timeslots, sum([(len(data.sol[i])) for i in range(data.Courses_max)]))
            SolNPLength = sum(
...
#while True:
#    output(PeraFile, message['Set'][-7:-1], Alpha, Beta, Gamma, Sigma, Delta, Epsilon, Initiate, CurrentObj, Iteration)
#result = outq.pop()
